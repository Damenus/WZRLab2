...
FILE *f = fopen("wwc__plik.txt","w");    // plik do zapisu informacji testowych
...
StanObiektu stan_z_ost_ramki[1000];
long czas_ost_ramki[1000];
...
// {czas [s], si³a [N], k¹t skrêtu kó³ [rad]} -> przez jaki czas obiekt ma sie poruszaæ z podan¹ prêdkoœci¹ i k¹tem skrêtu kó³
//float scenariusz_testu[][3] = {{5, 30, 0}, {5, 30, PI*20/180}, {5, 30, -PI*20/180}, {5, -10, 0}}; 
float scenariusz_testu[][3] = {{20, 50, 0}, {10, 0, PI*20/180}, {10, 30, -PI*20/180}}; 
//float scenariusz_testu[][3] = {{50, 100, 0}}; 
long nr_akcji = 0;
...
DWORD WINAPI WatekOdbioru(void *ptr)
{
  ...
  while(1)
  {
    ...
    //if (stan.iID != pMojObiekt->iID)          // jeœli to nie mój w³asny obiekt
    {
      ...
      stan_z_ost_ramki[IndeksyOb[stan.iID]] = stan;
      czas_ost_ramki[IndeksyOb[stan.iID]] = clock();
...
void PoczatekInterakcji()
{
  ...
  multi_reciv = new multicast_net("224.12.12.03",10001);      // obiekt do odbioru ramek sieciowych
  multi_send = new multicast_net("224.12.12.03",10001);       // obiekt do wysy³ania ramek
...
void Cykl_WS()
{
  ...
  long indeks_cienia = -1;         // indeks odbicia sieciowego w³asnego pojazdu
  for (long i=0;i<iLiczbaCudzychOb;i++) if (CudzeObiekty[i]->iID == pMojObiekt->iID) indeks_cienia = i;
  if (indeks_cienia > -1)
  {
    float rozmiar_x = teren.lwierszy*teren.rozmiar_pola,
      rozmiar_z = teren.lkolumn*teren.rozmiar_pola;
    float dx = CudzeObiekty[indeks_cienia]->wPol.x - pMojObiekt->wPol.x;
    if (dx > rozmiar_x/2) dx = rozmiar_x-dx;
    if (dx < - rozmiar_x/2) dx = rozmiar_x+dx;
    float dz = CudzeObiekty[indeks_cienia]->wPol.z - pMojObiekt->wPol.z;
    if (dz > rozmiar_z/2) dz = rozmiar_z-dz;
    if (dz < - rozmiar_z/2) dz = rozmiar_z+dz;
    float dy = CudzeObiekty[indeks_cienia]->wPol.y - pMojObiekt->wPol.y;
    sum_roznic_pol += sqrt(dx*dx+dy*dy+dz*dz);
  }
  ...
  {                              // nale¿y na nowo obliczyæ œredni czas cyklu fDt
    ...
    sprintf(text,"2017,wersja h, œr.czêstoœæ = %0.2f[r/s]  œr.odl = %0.3f[m]  œr.ro¿n.k¹t. = %0.3f[st]",
      sr_czestosc,sum_roznic_pol/licznik_sym,sum_roznic_kat/licznik_sym*180.0/3.14159);
